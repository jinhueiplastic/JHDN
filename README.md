...
2. 依序貼上並執行下面這些檔案的完整內容：
   ...
   - [`supabase/migrations/0018_driver_soft_delete.sql`](./supabase/migrations/0018_driver_soft_delete.sql)（司機改成軟刪除，加上 active 欄位）
   - [`supabase/migrations/0019_driver_sort_order.sql`](./supabase/migrations/0019_driver_sort_order.sql)（新增司機排序欄位，並把現有司機依姓名排出初始順序）
...
「已回單」分頁的「價格」欄位要按「儲存」數字才會真的寫入（下拉選單本身選了會馬上記
起來，但底下的數字/日期要按儲存）。按下「儲存」成功後會變成純文字顯示（不再是下拉
選單和輸入框），這樣才看得出來是不是真的存檔成功了；要改的話按旁邊的「編輯」再變回
可編輯狀態。金額輸入框打完按 Enter 等於直接按「儲存」（未處理/未回單分頁本來就是
打完離開輸入框就自動存檔，按 Enter 一樣有效）。

勾選「外縣市」後下面會多出「原因:」「件數:」「運費:」三個輸入框跟一個「儲存」按鈕，
填完要按「儲存」才會真的寫入（不會像其他欄位打完字、點掉就自動存檔）。按下「儲存」
成功後這三欄會變成純文字顯示（不再是輸入框），這樣才看得出來是不是真的存檔成功了；
要改的話按旁邊的「編輯」再變回輸入框。取消勾選外縣市時三個欄位會自動清空；三個
輸入框按 Enter 會依序跳到下一個（原因→件數→運費），在「運費」按 Enter 等於直接按
「儲存」。運費常常要晚一點才知道金額，沒有的話可以先不填、之後再補上（這種情況會
一直停留在輸入框狀態，直到真的按下儲存）。
...
- **已回單**：司機那一欄預設只顯示目前指派的司機姓名（粗體純文字），不是一排按鈕，
  要先按「編輯」才會展開成所有司機的按鈕清單可以改選，選了會跳出確認視窗（避免不
  小心點錯已經確認過的紀錄）；如果點錯狀態可以用「改回未回單」修正（一樣要等 1 秒
  才會真的轉換、跳走）；`unreturned_date` 這欄位在已回單頁面會改標示成「已回單
  日期」，記錄確認回單的日期；已回單狀態下修改「價格」欄不會像其他狀態一樣馬上
  自動存檔，要另外按「儲存」才會真的寫入，避免手滑改到已經確認過的金額
- **作廢**：司機那一欄也只顯示純文字（沒有編輯選項，作廢的單不能再改司機）
...
畫面上方可依日期查詢、依狀態篩選，預設顯示的分頁順序是：未處理/未回單/已回單/填單價/
填運費，後面接一個「...」按鈕，按下才會多顯示「作廢」「全部」這兩個比較少用的分頁
（再按一次「‹」收回去）。預設打開「未處理」。「填單價」是價格下拉選單選了「填單價」、
但還沒有實際填數字的訂單（選了現銷價或發票金額的不會算在這裡，要選過填單價才算）；
這個分頁在打開的當下會先「凍結」名單，之後在同一次停留期間修改某一列的下拉選單或
打完數字，那一列不會立刻從畫面上消失，避免打到一半列就跳掉；要切到別的分頁再切回來，
或換日期，名單才會重新整理；
「填運費」是所有勾選外縣市的訂單（不分運費有沒有填，填了也不會從這個分頁消失，
方便回頭確認/修改；作廢的單不算在這裡）；「全部」是不分狀態的完整清單。如果重新
整理前是停在「作廢」或「全部」，重新整理後這兩個分頁會自動保持展開，不會被「...」
擋住。分頁旁邊如果有已刪除的司機，也會有一個「舊司機...」按鈕，按下才會在司機選擇
區多顯示已刪除的司機（虛線框樣式）。每一列直接在表格上編輯，不需要另外開視窗。
...
- **`/filter`**（也可以用 `/filter.html`）：依司機查詢，點選司機名字看他所有日期的
  單號，可以選填起始/結束日期縮小範圍；司機按鈕旁邊有一個「舊司機...」按鈕，按下才
  會多顯示已刪除的司機（虛線框樣式）——刪除司機現在是「軟刪除」（`active` 欄位設成
  false），不是真的刪除那一列，訂單的司機欄位存的是純文字、不是連到司機名單表，
  所以刪除司機完全不影響已經指派給他的訂單，只是預設不會出現在按鈕清單裡，需要的話
  按「舊司機...」還是選得到，也可以在「司機名單管理」按「↺」恢復成正常司機；頁面
  最下方收合的「司機名單管理」可以新增/刪除司機，也可以用「↑↓」調整司機順序，這個
  順序會套用到`/daily`、`/filter`、`/report`所有選司機的地方（點旁邊的 ▾ 展開）
- **`/order`**（也可以用 `/order.html`）：依單號查詢，輸入完整 11 碼單號（例如
  `11507140001`）查單筆資料；只輸入 7 碼日期（例如 `1150714`）則查出當天全部單號
- **`/report`**（也可以用 `/report.html`）：列印報表，可以選「單日報表」（選一個
  日期，該天全部單號）或「依司機報表」（選司機，日期選填），按「下載 Excel」
...
## 4. Google Sheet 同步（Apps Script，自動排隊處理 + 異動歷史紀錄）
...
「每分鐘」自動執行的時間觸發條件，依序、一列一列把佇列清空，每筆異動會同時
寫進兩個分頁：

- **現況分頁**（預設分頁名稱 `Supabase同步(勿手動編輯)`）：每張訂單只保留
  最新的樣子，跟網站上看到的一致
- **異動紀錄分頁**（預設分頁名稱 `異動紀錄(勿手動編輯)`）：**只會增加、不會
  覆蓋或刪除**，每一次新增/修改/作廢都會在這裡多一列，記錄異動時間、類型、
  當下完整的欄位內容——就算現況分頁的資料被誤操作清空或改錯（例如不小心對
  某一天全選按了「設為原始狀態」），也可以回頭查這份紀錄，找出資料是什麼
  時候、被改成什麼樣子，藉此判斷是誤操作還是其他原因造成的

這兩個分頁都不要手動輸入資料，你自己的「出貨單」「Data」分頁不受影響。
...
異動紀錄分頁的欄位順序，是在最前面多兩欄，後面接上跟現況分頁一樣的 O 個欄位：

| 欄 | 欄位名稱 | 說明 |
|---|---|---|
| A | 記錄時間 | 這筆異動被 Apps Script 處理、寫進 Sheet 的時間（不是資料庫的 created_at/updated_at） |
| B | 異動類型 | 新增 / 更新 / 刪除 |
| C 開始 | （同現況分頁的 A~O 欄） | 那次異動當下、這張訂單完整的欄位內容 |
...
2. 左側「專案設定」(齒輪) -> 指令碼屬性，新增：
   ...
   - `SHEET_NAME`（選填，不填就用預設現況分頁名稱）
   - `LOG_SHEET_NAME`（選填，不填就用預設異動紀錄分頁名稱）
3. 執行一次 `syncFromSupabase`，同意授權視窗（順便把現有資料整批同步一次到
   現況分頁；這一步不會寫進異動紀錄分頁）
